...
def interpolate_funct(img, original_voxel_size, new_voxel_size, target_nb_pixels):
    ...
    y = xp.linspace(-1 / 2 * (original_shape[1] - 1) * original_voxel_size[1], 1 / 2 * (
        original_shape[1] - 1) * original_voxel_size[1], original_shape[1])
    z = xp.linspace(-1 / 2 * (original_shape[2] - 1) * original_voxel_size[2], 1 / 2 * (
        original_shape[2] - 1) * original_voxel_size[2], original_shape[2])

    # Create the new grid with the desired voxel size
    x_new = xp.linspace(-1 / 2 * (target_nb_pixels[0] - 1) * new_voxel_size[0], 1 / 2 * (
        target_nb_pixels[0] - 1) * new_voxel_size[0], target_nb_pixels[0])
    y_new = xp.linspace(y[-1] - (target_nb_pixels[1] - 1)
    ...
    x_new_mesh, y_new_mesh, z_new_mesh = xp.meshgrid(
        x_new, y_new, z_new, indexing='ij')

    # Generate the new volume by interpolating on the new grid
    new_points = xp.stack((x_new_mesh, y_new_mesh, z_new_mesh), axis=-1)
